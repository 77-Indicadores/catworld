...
| `DATA_INTEGRITY_SUSPECT` | carga barrada ou marcada como possivelmente incompleta pela barra de integridade (`success=false`); ver `docs/data-integrity.md` | veredito, dataset, tabela, esperado/lido/anterior, motivos estruturados |
| `INTEGRITY_SETTINGS_CHANGED` | política de integridade alterada (`mode`, `max_drop_pct`, `allow_empty`) | valores novos |
