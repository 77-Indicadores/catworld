...
- **Origem de `removedKeys`:** linhas excluídas na origem ficam **marcadas** (`cw_deleted_at`, a linha permanece; ver `source-contract.md`) e `removedKeys` = chaves marcadas com `cw_deleted_at > since`. Chave que voltou a existir na origem é reativada (a marca cai) e chega como linha alterada. Linhas marcadas não aparecem nas linhas devolvidas, no export, no OData nem nas consultas SQL.
