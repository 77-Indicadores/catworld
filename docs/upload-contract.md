...
- XLSX (uma aba por arquivo, estrito por padrao): dados em mais de uma aba (ou so na 2a aba) sao RECUSADOS com a lista das abas e o numero de linhas de cada uma; formula sem valor calculado no arquivo e recusada nomeando a celula (abra e salve no Excel, ou cole como valores); celulas mescladas: so a celula mestre tem valor (as demais ficam vazias, nao repetem o valor). Nao ha opcao por upload para escolher a aba: e preciso deixar so a aba desejada com dados (como primeira aba) ou exportar cada aba como CSV.
