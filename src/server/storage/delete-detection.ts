...
export function carryPlan(o: {
  ...
  if (o.qKeys && o.beforeParam) {
    const inKeys = `EXISTS (SELECT 1 FROM ${o.qKeys} k WHERE k.${key} = t.${key})`;
    const newly = `${td} IS NULL AND ${ts} < ${o.beforeParam} AND NOT ${inKeys}`;
    return {
  ...
  return { syncedAtExpr: ts, deletedAtExpr: td, markedWhere: null as string | null };
}
