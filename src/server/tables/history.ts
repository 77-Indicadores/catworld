...
export type HistoryVersion = {
  ...
  rowCount: string;
  origin: "upload" | "sync";
  upload: { filename: string; mode: string; createdBy: string | null } | null;
};
...
export type UploadRow = { id: string; originalFilename: string; mode: string; createdBy: string | null };
...
export function buildTableHistory(
  input: { versions: VersionRow[]; uploads: UploadRow[]; metrics: MetricRow[]; jobs: JobRow[] },
  ...
  const versions: HistoryVersion[] = [...input.versions]
    ...
    .map((v) => {
      ...
      return {
        ...
        upload: u ? { filename: u.originalFilename, mode: u.mode, createdBy: u.createdBy } : null,
      };
    });
...
export async function loadTableHistory(tableId: string, limit: number = HISTORY_LIMIT): Promise<TableHistory> {
  ...
  const [uploads, metrics, tableUploads, source, derived] = await Promise.all([
    uploadIds.length ? prisma.upload.findMany({ where: { id: { in: uploadIds } }, select: { id: true, originalFilename: true, mode: true, createdBy: true } }) : Promise.resolve([]),
  ...
  const jobs = or.length
    ...
    : [];
  return buildTableHistory({ versions, uploads, metrics, jobs }, limit);
}
