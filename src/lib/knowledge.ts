...
export const articles: Article[] = [
  ...
  {
    slug: "linguagem-sql",
    title: "Linguagem SQL (guia com exemplos)",
    description: "Como escrever consultas no Catworld: T-SQL, exemplos prontos e o que não é suportado.",
    category: "dados",
    icon: "Code",
    sections: [
      { kind: "text", text: "Em qualquer lugar do Catworld onde você escreve uma consulta — o painel de consulta, tabelas derivadas, exportações e o SDK — a linguagem é a mesma: T-SQL, o SQL do SQL Server. O Catworld traduz a consulta para o banco onde os dados estão guardados, então você escreve uma vez só." },
      { kind: "note", variant: "info", text: "Os exemplos abaixo usam o schema meu_schema e a tabela vendas só como ilustração. Troque pelos nomes do seu dataset (o navegador de tabelas do painel de consulta insere o nome certo com um clique)." },
      { kind: "heading", text: "O básico" },
      { kind: "text", text: "Toda consulta é somente leitura: uma única instrução começando por SELECT ou WITH. Comandos que alteram dados (INSERT, UPDATE, DELETE, DROP…) são recusados." },
      { kind: "code", label: "Exemplo — as 100 vendas mais recentes", value: "SELECT TOP 100 *\nFROM meu_schema.vendas\nORDER BY data_venda DESC" },
      { kind: "code", label: "Exemplo — total por ano", value: "SELECT YEAR(data_venda) AS ano,\n       COUNT(*)        AS pedidos,\n       SUM(valor)      AS total\nFROM meu_schema.vendas\nGROUP BY YEAR(data_venda)\nORDER BY ano" },
      { kind: "code", label: "Exemplo — juntando duas tabelas", value: "SELECT f.nome AS filial, SUM(v.valor) AS total\nFROM meu_schema.vendas v\nJOIN meu_schema.filiais f ON f.id = v.id_filial\nWHERE v.status = 'ATIVO'\nGROUP BY f.nome\nHAVING SUM(v.valor) > 10000\nORDER BY total DESC" },
      { kind: "heading", text: "Consultas em etapas (CTE) e funções de janela" },
      { kind: "code", label: "Exemplo — ranking de filiais", value: "WITH por_filial AS (\n  SELECT id_filial, SUM(valor) AS total\n  FROM meu_schema.vendas\n  GROUP BY id_filial\n)\nSELECT id_filial,\n       total,\n       RANK() OVER (ORDER BY total DESC) AS posicao\nFROM por_filial" },
      { kind: "code", label: "Exemplo — páginas de 50 linhas", value: "SELECT id, data_venda, valor\nFROM meu_schema.vendas\nORDER BY id\nOFFSET 100 ROWS FETCH NEXT 50 ROWS ONLY" },
      { kind: "note", variant: "tip", text: "OFFSET … FETCH exige ORDER BY. Para só limitar, TOP n é mais simples e também funciona dentro de subconsultas e CTEs." },
      { kind: "heading", text: "Datas" },
      { kind: "code", label: "Exemplo — últimos 30 dias", value: "SELECT COUNT(*) AS pedidos\nFROM meu_schema.vendas\nWHERE data_venda >= DATEADD(day, -30, GETDATE())" },
      { kind: "code", label: "Exemplo — dias entre duas datas", value: "SELECT id,\n       DATEDIFF(day, data_venda, data_entrega) AS dias_para_entregar\nFROM meu_schema.vendas" },
      { kind: "code", label: "Exemplo — data como texto (31/12/2026)", value: "SELECT CONVERT(VARCHAR(10), data_venda, 103) AS data_br\nFROM meu_schema.vendas" },
      { kind: "note", variant: "warning", text: "DATEDIFF conta as fronteiras cruzadas, como no SQL Server: de 31/12 às 23:59 para 01/01 às 00:01 é 1 dia, mesmo com 2 minutos de diferença. Unidades aceitas: year, quarter, month, week, weekday, dayofyear, day, hour, minute, second. Semana e dia da semana seguem o SQL Server (domingo = 1)." },
      { kind: "heading", text: "Texto, números e nulos" },
      { kind: "code", label: "Exemplo — funções comuns", value: "SELECT ISNULL(apelido, nome)                AS nome_exibicao,\n       LEN(nome)                           AS tamanho,\n       CONCAT(nome, ' - ', cidade)         AS rotulo,\n       IIF(valor > 1000, 'alto', 'normal') AS faixa,\n       CAST(valor AS DECIMAL(18, 2))       AS valor_2casas\nFROM meu_schema.clientes" },
      { kind: "text", text: "CAST e CONVERT para INT, BIGINT e SMALLINT truncam (2,7 vira 2), como no SQL Server. Para concatenar texto, prefira CONCAT a usar o sinal de +." },
      { kind: "code", label: "Exemplo — converter sem quebrar a consulta (NULL quando não for número)", value: "SELECT TRY_CAST(codigo_texto AS INT) AS codigo\nFROM meu_schema.produtos" },
      { kind: "heading", text: "Comportamentos que surpreendem" },
      {
        kind: "table",
        headers: ["No SQL Server", "No Catworld", "Como agir"],
        rows: [
          ["NULL vem primeiro no ORDER BY crescente", "Igual: o Catworld reproduz", "Nada a fazer"],
          ["LIKE ignora maiúsculas e minúsculas", "Igual: o Catworld reproduz", "Nada a fazer"],
          ["=, IN, GROUP BY e JOIN em texto ignoram maiúsculas e minúsculas", "Podem diferenciar: WHERE nome = 'BIA' não acha 'Bia' em dados no Postgres", "Use UPPER(nome) = 'BIA' ou LIKE"],
          ["Ordem alfabética com acentos (collation)", "Pode ser diferente", "Confira a ordem quando ela importar"],
          ["Conversão implícita entre texto, número e data", "Pode dar erro", "Use CAST ou CONVERT explícitos"],
        ],
      },
      { kind: "heading", text: "O que não funciona" },
      { kind: "text", text: "Estes recursos não fazem parte da linguagem do Catworld. Quando você usa um deles, o Catworld responde com o erro UNSUPPORTED_CONSTRUCT citando o trecho — não um erro cru do banco." },
      {
        kind: "table",
        headers: ["Não use", "Faça assim"],
        rows: [
          ["PIVOT / UNPIVOT", "SUM(CASE WHEN … THEN … END) para cada coluna"],
          ["FOR XML / FOR JSON", "Monte o formato no seu sistema (o resultado já vem em JSON)"],
          ["OPENQUERY / OPENROWSET", "Crie uma fonte conectada e consulte a tabela sincronizada"],
          ["TOP n PERCENT", "TOP com um número fixo de linhas"],
          ["TRY_PARSE; TRY_CAST/TRY_CONVERT para data ou bit", "CAST/CONVERT, tratando os valores inválidos no WHERE"],
          ["DATEDIFF em millisecond", "DATEDIFF em second"],
          ["CONVERT com estilo fora da lista (8, 20, 21, 23, 24, 101–105, 108, 110–112, 120, 121, 126, 127)", "Um estilo da lista"],
          ["Sintaxe de Postgres, como valor::tipo", "CAST(valor AS tipo)"],
        ],
      },
      { kind: "code", label: "Não funciona — cast no estilo Postgres", value: "SELECT valor::int FROM meu_schema.vendas" },
      { kind: "code", label: "Alternativa", value: "SELECT CAST(valor AS INT) FROM meu_schema.vendas" },
      { kind: "heading", text: "O resultado" },
      {
        kind: "table",
        headers: ["Tipo", "Como chega"],
        rows: [
          ["Data", "\"AAAA-MM-DD\""],
          ["Data e hora", "ISO-8601 em UTC"],
          ["Hora", "\"HH:MM:SS\""],
          ["Números grandes e decimais", "Texto (string), para não perder precisão"],
        ],
      },
      { kind: "text", text: "Se havia mais linhas do que o limite, o resultado vem com truncated = true. Um TOP que você mesmo escreveu nunca marca o resultado como truncado." },
      { kind: "heading", text: "Acesso e linhas excluídas" },
      {
        kind: "list",
        items: [
          "Você só enxerga os datasets a que tem acesso; qualificar o nome de outro schema dá erro de permissão.",
          "Linhas apagadas na origem e marcadas pelo Catworld nunca aparecem nas suas consultas, nem em exportações e tabelas derivadas.",
          "SELECT * inclui as colunas cw_synced_at (quando a linha foi sincronizada) e cw_deleted_at (sempre vazia nas consultas).",
        ],
      },
      { kind: "note", variant: "info", text: "O SQL nativo de uma fonte por consulta (sourceSql) roda direto no banco de origem, no dialeto dele, e não é traduzido." },
    ],
  },
  // ── SEGURANÇA ────────────────────────────────────────────────────────
  {
